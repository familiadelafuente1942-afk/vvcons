    ...
    return (<div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        <div style={{ background: T.navy, padding: "16px 18px 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginTop: 16 }}>
                {[{ l: t(cfg, 'dash_proyectoes'), v: lics.filter(l => !["adjudicada", "descartada"].includes(l.estado)).length, c: "#60A5FA" }, { l: t(cfg, 'dash_obras_activas'), v: obras.filter(o => o.estado === "curso").length, c: "#34D399" }, { l: t(cfg, 'dash_alertas'), v: alerts.length, c: "#FBBF24" }, { l: t(cfg, 'dash_personal'), v: personal.length, c: "#A78BFA" }].map(k => (
...
function Personal({ personal, setPersonal, obras, cfg }) {
    ...
    const [showNew, setShowNew] = useState(false);
    const [form, setForm] = useState({ nombre: "", rol: "Técnico", empresa: "V+V Construcciones", obra_id: "", telefono: "", foto: "", tareas: [] });

    // Cargar datos de presentismo para ver historial
    useEffect(() => {
        (async () => {
            try {
                const r = await storage.get('bco_presentismo');
    ...
    function add() { if (!form.nombre.trim()) return; setPersonal(p => [...p, { ...form, id: uid(), docs: {} }]); setForm({ nombre: "", rol: "Técnico", empresa: "V+V Construcciones", obra_id: "", telefono: "", foto: "", tareas: [] }); setShowNew(false); }
    ...
    return (<div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        {showNew && (<Sheet title={t(cfg, 'pers_nuevo')} onClose={() => setShowNew(false)}>
            ...
            <FieldRow>
                ...
                <Field label={t(cfg, 'pers_empresa')}><TInput value={form.empresa} onChange={e => setForm(p => ({ ...p, empresa: e.target.value }))} placeholder="V+V Construcciones" /></Field>
            </FieldRow>
...
Formato profesional V+V Construcciones, español rioplatense.` });
...
function PanelVigilancia({ setView }) {
    const [camaras, setCamaras] = useStoredState('bco_camaras', []);
    const [showNew, setShowNew] = useState(false);
    const [form, setForm] = useState({ nombre: '', url: '', sector: '', ap: 'norte', tipo: 'ip' });
    function add() { if (!form.nombre || !form.url) return; setCamaras(p => [...p, { ...form, id: uid() }]); setForm({ nombre: '', url: '', sector: '', ap: 'norte', tipo: 'ip' }); setShowNew(false); }
    ...
    return (<div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {showNew && (<Sheet title="Agregar cámara" onClose={() => setShowNew(false)}>
            ...
            <FieldRow>
                <Field label="zona"><Sel value={form.ap} onChange={e => setForm(p => ({ ...p, ap: e.target.value }))}>{AIRPORTS.map(a => <option key={a.id} value={a.id}>{a.code}</option>)}</Sel></Field>
...
function Presentismo({ personal, setPersonal, obras, setObras, currentUser, setView }) {
    const [presentismoData, setPresentismoData] = useStoredState('bco_presentismo', { registros: {}, bioLink: '', trackingAuto: false });
...
function Archivos({ setView }) {
    const [files, setFiles] = useStoredState('bco_archivos', []);
...
function CotizacionView({ setView, apiKey, cfg }) {
    ...
    const [historial, setHistorial] = useStoredState('bco_cotizaciones', []);
    const camRef = useRef(null);
    const galRef = useRef(null);
...
<html lang="es">
<head><meta charset="UTF-8"><title>Cotización V+V Construcciones</title>
<style>
...
<div class="meta">
  <b>V+V Construcciones</b> &nbsp;|&nbsp; Fecha: ${item.fecha} ${item.hora || ''} &nbsp;|&nbsp;
...
<div class="footer">Generado por V+V Construcciones — Precios de referencia, consultar con proveedores para cotización final.</div>
</body></html>`;
...
function MensajesView({ setView, currentUser, personal }) {
    ...
    async function loadMensajes() {
        try {
            const r = await storage.get('bco_mensajes');
    ...
    async function enviar() {
        ...
        try { await storage.set('bco_mensajes', JSON.stringify(actualizados)); } catch { }
    }
...
function ContactosView({ setView }) {
    const [contactos, setContactos] = useStoredState('bco_contactos', []);
    const [showNew, setShowNew] = useState(false);
    ...
    return (<div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        {showNew && (<Sheet title="Nuevo contacto" onClose={() => setShowNew(false)}>
            ...
            <Field label="Empresa"><TInput value={form.empresa} onChange={e => setForm(p => ({ ...p, empresa: e.target.value }))} placeholder="V+V Construcciones" /></Field>
            <FieldRow>
...
function ProveedoresView({ setView }) {
    const [provs, setProvs] = useStoredState('bco_proveedores', []);
    const [showNew, setShowNew] = useState(false);
...
function InformesIA({ obras, setObras, setView, apiKey }) {
    ...
    async function generar() {
        ...
        const template = tipo === 'diario'
            ? `Generá un INFORME DIARIO para "${obra.nombre}" (V+V Construcciones ${AIRPORTS.find(a => a.id === obra.ap)?.code || obra.ap}). Avance actual: ${obra.avance}%. Fecha: ${new Date().toLocaleDateString('es-AR')}.
...
Tono profesional V+V Construcciones, español rioplatense.`
            : `Generá un INFORME SEMANAL para "${obra.nombre}" (V+V Construcciones ${AIRPORTS.find(a => a.id === obra.ap)?.code || obra.ap}). Avance: ${obra.avance}%. Semana del ${new Date().toLocaleDateString('es-AR')}.
...
Tono profesional V+V Construcciones, español rioplatense.`;
        const r = await callAI([{ role: 'user', content: template }],
            'Sos ingeniero de obra para V+V Construcciones. Generás informes técnicos claros y profesionales en español rioplatense. Si el informe requiere datos de precios o mercado, buscalos en internet.', apiKey, true);
...
function Chat({ lics, obras, setObras, personal, alerts, cfg, apiKey }) {
    ...
    const [msgs, setMsgs] = useState(() => {
        try {
            const saved = localStorage.getItem('bco_chat_msgs');
            if (!saved) return [];
    ...
    const [userName, setUserName] = useState(() => {
        try { return localStorage.getItem('bco_chat_user') || ''; } catch { return ''; }
    });
    const [askedName, setAskedName] = useState(() => {
        try { return !!localStorage.getItem('bco_chat_user'); } catch { return false; }
    });
    ...
    useEffect(() => {
        ...
        try {
            ...
            localStorage.setItem('bco_chat_msgs', JSON.stringify({ msgs: msgsLimpios, lastAt: Date.now() }));
        } catch { }
    }, [msgs]);
    ...
    function limpiarChat() {
        setMsgs([]);
        try { localStorage.removeItem('bco_chat_msgs'); } catch { }
    }
    ...
    useEffect(() => {
        (async () => {
            try {
                const r = await storage.get('bco_chat_user');
                if (r?.value && r.value !== userName) {
                    setUserName(r.value);
                    setAskedName(true);
                    try { localStorage.setItem('bco_chat_user', r.value); } catch { }
                }
            } catch { }
            setChatLoaded(true);
        })();
    ...
    function buildContext(txt) {
        ...
        const base = `Empresa: ${cfg.empresa || 'V+V Construcciones'} · ${cfg.cargo || ''} · ${new Date().toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' })}`;
        ...
        if (quiereLics || quiereTodo) {
            ...
            ctx += `\nPROYECTOS activas (${licsActivas.length}):\n`;
    ...
    async function enviar() {
        ...
        if (!askedName && !userName) {
            ...
            setUserName(txt);
            setAskedName(true);
            try { await storage.set('bco_chat_user', txt); } catch { }
            try { localStorage.setItem('bco_chat_user', txt); } catch { }
            setInput('');
            setTimeout(() => setMsgs(p => [...p, { id: uid(), role: 'assistant', text: `Hola ${txt}, soy tu asistente IA para V+V Construcciones. Tengo acceso en tiempo real a todas tus obras, proyectoes, personal y alertas. ¿En qué puedo ayudarte?` }]), 400);
            return;
        }
        ...
        const sys = `Sos el asistente IA de V+V Construcciones para construcción en zonas V+V Construcciones.
...
Respondé en español rioplatense. Sé conciso y directo — máximo 3-4 párrafos salvo que pidan un informe detallado.${usarBusqueda ? ' Buscá en internet para dar precios y datos actualizados.' : ''}`;
    ...
    async function guardarEnArchivos(att) {
        try {
            ...
            const localVal = storage.getLocal('bco_archivos');
            ...
            try { localStorage.setItem('bco_archivos', JSON.stringify(arr)); } catch { }
            storage.set('bco_archivos', JSON.stringify(arr)).catch(() => {});
        } catch { }
        setShowSaveDialog(null);
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", paddingBottom: 68 }}>
            {msgs.length === 0 ? (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "30px 24px", textAlign: "center", minHeight: "100%" }}>
                    ...
                    {userName && <button onClick={async () => { setUserName(''); setAskedName(false); try { await storage.delete('bco_chat_user'); } catch { } try { localStorage.removeItem('bco_chat_user'); } catch { } }} style={{ background: "none", border: "none", color: T.muted, fontSize: 12, cursor: "pointer", textDecoration: "underline", marginTop: 20 }}>
                        No soy {userName}
                    </button>}
                </div>
...
function AlertasWA({ cfg, personal, lics, obras, alerts, setView }) {
    ...
    const TIPOS = [
        ...
        { id: 'proyectoes', label: 'Proyectos urgentes', color: '#3B82F6', bg: '#EFF6FF' },
    ...
    function getMensajesParaTipo(tipo) {
        ...
        switch (tipo) {
            case 'criticas':
                return alerts.filter(a => a.prioridad === 'alta').map(a => `⚠ V+V Construcciones — ALERTA CRÍTICA\n${a.msg}\nFecha: ${hoy}`);
            case 'documentacion':
                return alerts.filter(a => a.id.startsWith('docfalta') || a.id.startsWith('doc_')).map(a => `📋 V+V Construcciones — Documentación\n${a.msg}\nPor favor regularizá esta situación. Fecha: ${hoy}`);
            case 'proyectoes':
                return alerts.filter(a => a.id.startsWith('lic_')).map(a => `🏗 V+V Construcciones — Proyecto\n${a.msg}\nFecha: ${hoy}`);
            case 'personalizada':
                return msgCustom.trim() ? [`📢 V+V Construcciones\n${msgCustom.trim()}\nFecha: ${hoy}`] : [];
            default: return [];
        }
    }
...
function Mas({ setView, setUser, user, cfg, setCfg, apiKey, setApiKey }) {
    ...
    function logout() {
        setUser(null);
        try { storage.delete('bco_current_user'); } catch { }
        setView('login');
    }
    ...
    return (<div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        {showCfg && (<Sheet title="Configuración" onClose={() => setShowCfg(false)}>
            ...
            {cfgSection === 'cuenta' && (<div>
                <Field label="Empresa"><TInput value={cfg.empresa || ''} onChange={e => updCfg({ empresa: e.target.value })} placeholder="V+V Construcciones" /></Field>
            ...
            {cfgSection === 'logos' && (<div>
                {[{ key: 'logoV+V Construcciones', l: 'Logo izquierdo' }, { key: 'logoEmpresa', l: 'Logo derecho' }, { key: 'logoAsistente', l: 'Logo asistente IA' }, { key: 'logoCentral', l: 'Logo login central' }].map(lg => (
                ...
                <Field label="Título asistente"><TInput value={cfg.tituloAsistente || ''} onChange={e => updCfg({ tituloAsistente: e.target.value })} placeholder="Asistente V+V Construcciones" /></Field>
            ...
            {cfgSection === 'ubic' && (<div>
                <Field label="Etiqueta del campo (ej: zona, Sucursal, Obra)"><TInput value={cfg.labelUbicacion || 'zona'} onChange={e => updCfg({ labelUbicacion: e.target.value })} /></Field>
                <Lbl>Ubicaciones</Lbl>
            ...
            {cfgSection === 'whatsapp' && (<div>
                ...
                <div style={{ marginTop: 14, background: T.bg, borderRadius: 10, padding: "12px 14px" }}>
                    ...
                    <div style={{ fontSize: 11, color: T.muted, lineHeight: 1.6 }}>
                        ...
                        3. Asignale el app V+V Construcciones con rol Admin<br/>
...
function LoginScreen({ onLogin, cfg, personal }) {
    ...
    return (<div style={{ height: "100vh", display: "flex", flexDirection: "column", background: T.bg, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px" }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: T.text, marginBottom: 4, letterSpacing: "-0.02em" }}>V+V Construcciones</div>
...
class ErrorBoundary extends React.Component {
    ...
    componentDidCatch(error, info) { console.error('[V+V Construcciones Error]', error, info); this.setState({ info }); }
    render() {
...
function AppInner() {
    ...
    const [user, setUser] = useState(() => getLocalJSON('bco_current_user', null));
    const [view, setView] = useState('chat');
    const [detailObraId, setDetailObraId] = useState(null);
    const [lics, setLics] = useState(() => getLocalJSON('bco_lics', []));
    const [obras, setObras] = useState(() => getLocalJSON('bco_obras', []));
    const [personal, setPersonal] = useState(() => getLocalJSON('bco_personal', []));
    const [alerts, setAlerts] = useState([]);
    const [cfg, setCfg] = useState(() => ({ ...DEFAULT_CONFIG, ...getLocalJSON('bco_cfg', {}) }));
    const [apiKey, setApiKey] = useState(() => getLocalStr('bco_api_key', ''));
    const [loaded, setLoaded] = useState(false);
        ...
        (async () => {
            try {
                ...
                const [rLics, rObras, rPers, rCfg, rKey, rUser] = await Promise.all([
                    storage.get('bco_lics'), storage.get('bco_obras'), storage.get('bco_personal'),
                    storage.get('bco_cfg'), storage.get('bco_api_key'), storage.get('bco_current_user'),
                ]);

                // Proyectos — solo actualizar si Supabase tiene datos y son distintos
                if (rLics?.value) try {
                    ...
                    if (licsBase.length > 0) {
                        ...
                        const licsConVisitas = await Promise.all(licsBase.map(async l => {
                            ...
                            const localVis = storage.getLocal(`bco_lic_vis_${l.id}`);
                            let visitas = localVis?.value ? JSON.parse(localVis.value) : (l.visitas || []);
                            try {
                                const rv = await storage.get(`bco_lic_vis_${l.id}`);
                                if (rv?.value) {
    ...
    // Persistir lics SIN visitas (las fotos van en bco_lic_vis_{id})
    useEffect(() => {
        if (!loaded) return;
        markLocalEdit('lics');
        const licsSinVisitas = lics.map(l => ({ ...l, visitas: [] }));
        const json = JSON.stringify(licsSinVisitas);
        storage.set('bco_lics', json).catch(() => { });
        try { localStorage.setItem('bco_lics', json); } catch { }
        // Guardar visitas de cada lic en su propia key
        lics.forEach(l => {
            if (!l.visitas?.length) return;
            const key = `bco_lic_vis_${l.id}`;
    ...
    useEffect(() => {
        ...
        storage.set('bco_obras', JSON.stringify(obrasSinMedia)).catch(() => { });
        try { localStorage.setItem('bco_obras', JSON.stringify(obrasSinMedia)); } catch { }
    }, [obras, loaded]);
    useEffect(() => { if (loaded) { markLocalEdit('personal'); storage.set('bco_personal', JSON.stringify(personal)).catch(() => { }); try { localStorage.setItem('bco_personal', JSON.stringify(personal)); } catch { } } }, [personal, loaded]);
    useEffect(() => { if (loaded) { markLocalEdit('cfg'); storage.set('bco_cfg', JSON.stringify(cfg)).catch(() => { }); try { localStorage.setItem('bco_cfg', JSON.stringify(cfg)); } catch { } } }, [cfg, loaded]);
    useEffect(() => {
        if (!loaded) return;
        // Solo guardar si la API key tiene contenido — no sobrescribir con vacío
        if (apiKey && apiKey.trim()) {
            storage.set('bco_api_key', apiKey).catch(() => { });
            try { localStorage.setItem('bco_api_key', apiKey); } catch { }
        }
    }, [apiKey, loaded]);
    useEffect(() => { if (loaded && user) { storage.set('bco_current_user', JSON.stringify(user)).catch(() => { }); try { localStorage.setItem('bco_current_user', JSON.stringify(user)); } catch { } } }, [user, loaded]);
    ...
    useEffect(() => {
        ...
        const MEDIA_PREFIXES = ['bco_fotos_', 'bco_archs_', 'bco_lic_vis_'];
        ...
        async function applyRemoteKey(key, value) {
            const now = Date.now();
            try {
                if (key === 'bco_lics' && now - myLastSave.lics > PROTECT_MS) {
                ...
                else if (key === 'bco_obras' && now - myLastSave.obras > PROTECT_MS) {
                ...
                else if (key === 'bco_personal' && now - myLastSave.personal > PROTECT_MS) {
                    const nv = JSON.parse(value); setPersonal(nv);
                    try { localStorage.setItem(key, value); } catch {}
                }
                else if (key === 'bco_cfg' && now - myLastSave.cfg > PROTECT_MS) {
                    const nv = JSON.parse(value); setCfg({ ...DEFAULT_CONFIG, ...nv });
                    try { localStorage.setItem(key, value); } catch {}
                }
                // Fotos de obras
                else if (key.startsWith('bco_fotos_')) {
                    const obraId = key.replace('bco_fotos_', '');
                    const fotos = JSON.parse(value);
                    setObras(cur => cur.map(o => o.id === obraId ? { ...o, fotos } : o));
                    try { localStorage.setItem(key, value); } catch {}
                }
                // Archivos de obras
                else if (key.startsWith('bco_archs_')) {
                    const obraId = key.replace('bco_archs_', '');
                    const archivos = JSON.parse(value);
                    setObras(cur => cur.map(o => o.id === obraId ? { ...o, archivos } : o));
                    try { localStorage.setItem(key, value); } catch {}
                }
                // Visitas de proyectoes
                else if (key.startsWith('bco_lic_vis_')) {
                    const licId = key.replace('bco_lic_vis_', '');
        ...
        async function syncAll() {
            try {
                ...
                const [rLics, rObras, rPers, rCfg] = await Promise.all([
                    storage.get('bco_lics'), storage.get('bco_obras'),
                    storage.get('bco_personal'), storage.get('bco_cfg'),
                ]);
                if (rLics?.value) { const loc = storage.getLocal('bco_lics'); if (loc?.value !== rLics.value) await applyRemoteKey('bco_lics', rLics.value); }
                if (rObras?.value) { const loc = storage.getLocal('bco_obras'); if (loc?.value !== rObras.value) await applyRemoteKey('bco_obras', rObras.value); }
                if (rPers?.value) { const loc = storage.getLocal('bco_personal'); if (loc?.value !== rPers.value) await applyRemoteKey('bco_personal', rPers.value); }
                if (rCfg?.value) { const loc = storage.getLocal('bco_cfg'); if (loc?.value !== rCfg.value) await applyRemoteKey('bco_cfg', rCfg.value); }

                // Sync fotos/visitas de cada obra y proyecto (media keys)
                setObras(cur => {
                    cur.forEach(async o => {
                        try {
                            const rf = await storage.get(`bco_fotos_${o.id}`);
                            if (rf?.value) { const loc = storage.getLocal(`bco_fotos_${o.id}`); if (loc?.value !== rf.value) await applyRemoteKey(`bco_fotos_${o.id}`, rf.value); }
                        } catch {}
                        try {
                            const ra = await storage.get(`bco_archs_${o.id}`);
                            if (ra?.value) { const loc = storage.getLocal(`bco_archs_${o.id}`); if (loc?.value !== ra.value) await applyRemoteKey(`bco_archs_${o.id}`, ra.value); }
                        } catch {}
                    });
                ...
                setLics(cur => {
                    cur.forEach(async l => {
                        try {
                            const rv = await storage.get(`bco_lic_vis_${l.id}`);
                            if (rv?.value) { const loc = storage.getLocal(`bco_lic_vis_${l.id}`); if (loc?.value !== rv.value) await applyRemoteKey(`bco_lic_vis_${l.id}`, rv.value); }
                        } catch {}
                    });
                    return cur;
                });
        ...
        // Supabase Realtime — escucha cambios en bco_storage en tiempo real
        ...
        function connectRealtime() {
            try {
                ...
                ws.onopen = () => {
                    setRealtimeOk(true);
                    // Suscribirse a cambios en la tabla bco_storage
                    ws.send(JSON.stringify({
                        topic: 'realtime:public:bco_storage',
                        event: 'phx_join',
                        payload: { config: { postgres_changes: [{ event: '*', schema: 'public', table: 'bco_storage' }] } },
                        ref: '1'
                    }));
        ...
        storage.set = async (key, value) => {
            if (key === 'bco_lics') myLastSave.lics = Date.now();
            else if (key === 'bco_obras') myLastSave.obras = Date.now();
            else if (key === 'bco_personal') myLastSave.personal = Date.now();
            else if (key === 'bco_cfg') myLastSave.cfg = Date.now();
    ...
    return (<>
        ...
        <div style={{ width: "100%", maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: T.bg, display: "flex", flexDirection: "column", position: "relative", color: T.text, fontFamily: "var(--font), sans-serif" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", paddingBottom: showNav ? 72 : 0 }}>
                ...
                {view === 'proyectoes' && <Proyectos lics={lics} setLics={setLics} requireAuth={requireAuth} cfg={cfg} obras={obras} setObras={setObras} />}
